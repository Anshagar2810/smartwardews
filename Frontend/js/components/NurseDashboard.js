                    ...
                    <div className="max-w-6xl mx-auto space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div className="bg-orange-50 rounded-xl p-6 flex flex-col items-center justify-center border border-orange-100 transform transition-all duration-300 hover:scale-105 hover:shadow-lg cursor-pointer" onClick={() => setSelectedMetric('spo2')}>
                                ...
                                <div className="text-3xl font-bold text-gray-800">{history.spo2 && history.spo2.length > 0 && history.spo2[history.spo2.length - 1] != null && !isNaN(history.spo2[history.spo2.length - 1]) ? `${history.spo2[history.spo2.length - 1]}%` : (selectedPatient?.vitals?.spo2 ? `${selectedPatient.vitals.spo2}%` : '-')}</div>
                            </div>
                            {/* Heart Rate */}
                            ...
                            <div className="bg-blue-50 rounded-xl p-6 flex flex-col items-center justify-center border border-blue-100 transform transition-all duration-300 hover:scale-105 hover:shadow-lg cursor-pointer" onClick={() => setSelectedMetric('temp')}>
                                ...
                                <div className="text-3xl font-bold text-gray-800">{history.temp && history.temp.length > 0 && history.temp[history.temp.length - 1] != null && !isNaN(history.temp[history.temp.length - 1]) ? `${history.temp[history.temp.length - 1]}°F` : (selectedPatient?.vitals?.temp ? `${selectedPatient.vitals.temp}°F` : '-')}</div>
                            </div>
                        </div>
