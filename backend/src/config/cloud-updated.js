...
export const fetchFromThingSpeak = async () => {
  try {
    ...
    // Find ALL patients with this deviceId (allow multiple patients per device)
    let patients = await Patient.find({ deviceId });
    
    if (patients.length === 0) {
      console.log(`⚠️ No patients found for deviceId: ${deviceId}. Using data for ALL patients...`);
      // Fallback: get ALL patients so everyone gets the sensor data
      patients = await Patient.find();
      if (patients.length === 0) return;
    }

    // Store vitals for ALL patients with this device
